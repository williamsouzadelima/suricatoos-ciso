...
export const load: PageServerLoad = async ({ fetch, params }) => {
	...
	const [dashRes, capRes, trRes, bdRes, csRes] = await Promise.all([
		...
		fetch(`${BASE_API_URL}/delivery/engagements/${params.id}/catalogs_summary/`)
	]);
	return {
		...
		catalogsSummary: csRes.ok ? await csRes.json() : null
	};
};
